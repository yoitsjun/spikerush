...
| Profile | client sends `"get"`, `("select", charId)`, `("upgrade", charId, stat, ±1\|5\|10)`, `("spin", banner, 1\|10)`, `("autoroll", banner)`, `"stop"`, `("autosell", rarity, on)`, `("equip", kind, key)`, `("buy", pack, "VP"\|"Gold")` (Studio only, packs without an id); server replies with a snapshot (with `reveal` after a spin) |
