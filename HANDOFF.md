...
**Assets.** The owner originally wanted The Spike's own sounds and visuals copied in. That was declined, and the project builds original equivalents instead: procedural effects and 28 synthesized sounds. Keep it that way.
...
- Vector Set: a Vector setter's sets carry `meta.vectorSet` (BallRenderer pulses the ball violet, VFX rings it). A spike whose `ctx.lastHit` is such a set of its own team gets `kmh x (1 + MaxBoost x k)`, `k` from the drop angle of the contact-to-landing line between `AngleMin` and `AngleMax`; `meta.vector`, `meta.vectorBoost` (the VFX popup "+x.x%").
...
- Jump sets: a set's `lift` (how far above a standing set it was taken; not for quicks) is added to the apex in `HitLogic.setArc`. BotService.`planJumpSet` times a setter's jump so it meets the pass at the top (`Bots.JumpSetChance`, `JumpSetPassDepth`); the jump sends ActionFX "Jump" "Set" and AnimationController holds `SetCatch` in the air.
...
- Counter Edge: a heavy, non-slide receive by a Counter character has no drain and can't be guard-broken; it returns `meta.counterGain` (`GainPerKmh` x the incoming km/h, `MinGain`..`MaxGain`). The server adds it to `entity.counter` (0..100, `TeamService.setCounter`, attribute `Counter` on the character and the player, reset each set); the next spike gets `kmh x (1 + MaxBoost x counter / 100)` and `meta.counterRelease`, and the meter empties. VFX: blades out and back in on the receive (`counterBlades`, sound `Blades`), a blade volley along the released spike.
