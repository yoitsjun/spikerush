...
- Counter Edge: a heavy, non-slide receive by a Counter character has no drain and can't be guard-broken; it returns `meta.counterGain` (`GainPerKmh` x the incoming km/h, `MinGain`..`MaxGain`). The server adds it to `entity.counter` (0..100, `TeamService.setCounter`, attribute `Counter` on the character and the player, reset each set, and it refreshes the boosts). She scales with it: `effectiveStats` adds `PerFull` (+55 Attack, +80 Defense) times the meter / 100 (`extra.counter`; ctx and `boostCtx` carry it). Ines' ceilings are low (150 / 120) so an empty meter is weak and a full one reaches 205 / 200. Her spikes flag `meta.counterEdge` (the meter) for the effects only. VFX: blades out and back in on the receive (`counterBlades`, sound `Blades`), a blade volley along her spikes that grows with the meter.
