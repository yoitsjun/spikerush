...
A loading screen (in ReplicatedFirst, all GUI shapes) shows soft grey light, rising bubbles, light rays and a spinning yellow, blue and white ball over the title, with tips and a progress bar, for at least three seconds and until the client has started. Then a short cinematic plays, like the recruit's: a yellow screen, a beam of light and your avatar's silhouette jumping to spike a ball over the net. It slows into a freeze just before the ball lands, with "Click to continue"; a click (or any key) plays the landing (a flash, rings and a bounce) and fades into the game. `tools/generate_icon.py` draws the game icon and thumbnail (`assets/icon/GameIcon.png`, 512x512, and `Thumbnail.png`, 1920x1080) in the same style; upload them in the Creator Hub.
