...
An anime volleyball game for Roblox in a 2.5D side view, modelled on the feel of *The Spike* (SUNCYAN): huge jumps, high sets, spikes measured in km/h, a team stamina "guard" meter, and characters whose tier and rolled stat caps set how strong they can be. Everything in this project is original. The court, effects, UI, loading screen, game icon and all 27 sound effects are procedural or synthesized from scratch, and nothing is taken from The Spike.
...
Player progress (V Points, every character's build and rolled caps, unlocked cosmetics) is saved with DataStoreService. In Studio this only works after enabling Game Settings > Security > "Enable Studio Access to API Services" on a published place. Without it the game still runs, profiles just last for the session, and the lobby says so.
...
## Characters, tiers and V Points

A character is a tier, a height, four stat caps and four stats: Attack, Defense, Speed and Jump. Every tier is its own saved character; pick one in the lobby and it becomes the one you play.

| Tier | D- | D | D+ | C- | C | C+ | B- | B | B+ | A- | A | A+ | S- | S | S+ |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Highest cap | 105 | 110 | 115 | 120 | 125 | 130 | 135 | 140 | 145 | 150 | 155 | 160 | 165 | 170 | 175 |
| Point total | 375 | 395 | 410 | 430 | 445 | 465 | 480 | 500 | 515 | 535 | 550 | 570 | 585 | 600 | 620 |

The tier sets two things: the point total you spread over the four stats, and how high a stat cap can roll. Each character's own four caps are rolled (V Points spins), each between 72% of the tier's highest cap and the cap itself, with top values rare. Points move freely: the Play tab has -10, -5, -1, +1, +5 and +10 on every stat, a free-points counter and an Auto button, and a stat can never pass its rolled cap. So two S+ characters differ by their rolls, and a lucky roll lets you place the whole 620. A new character comes with a rolled height and caps and its points already spread evenly.

Stats map onto gameplay the same way at every tier, from 50 up to 175. Attack sets a power multiplier from 0.45 to 1.00 for spikes and serves. Jump adds 0.25 to 1.55 m of vertical on top of your standing reach and makes the run-up faster and longer. Defense sets the team stamina pool (50 to 100), cuts stamina drain by up to 35%, and improves receives and blocks. Speed sets run speed (22 to 32) and set accuracy.

Height rolls on a bell curve around 185 cm between 165 and 205. Standing reach is 1.3 cm of reach per cm of height, and your hitting point at the top of a jump is standing reach plus your Jump vertical. So a maxed D- hits about 3.2 m and a maxed S+ at 185 cm about 3.95 m. Only the very top clears the 4.00 m Thunder line: a maxed S+ from about 189 cm, a maxed S from about 193 cm. The readout shows your hand's height: a ball met above your hand still counts as your own top.

The world is built at *The Spike*'s scale (4.6 studs to the metre, a real 9 m half court, a 2.43 m net) with characters about 1.15 m tall, and heights above the standing hand are drawn 1.4 times taller (`Config.Scale.JumpScale`), so a maxed S+ leaps almost three times its own height and hits at twice the net while the readouts stay in real metres. Workspace gravity is 45 and the ball falls at 11.25 m/s².

**V Points (VP)** are the currency. You earn 30 for a win, 15 for a loss and 2 per kill, ace or block, and a new profile starts with 500. The Shop tab spends them on spins: x1 costs 50 VP and x10 costs 500.

| Spin | What it rolls |
|---|---|
| Stat caps | This character's four caps. Graded Common to Legendary by how close they are to the tier cap |
| Height | This character's height. 200 cm and up is Legendary (about 1 in 20) |
| Spike style | Spike animations: Full Bow, Scissor Kick, Double Hammer, Whirlwind |
...
| Score effect | What happens where your attack lands for a point: Shockwave, Fire Explosion, Meteor Strike, Thunderbolt |

Stat-cap and height spins show their results and you keep one (x10 shows ten) or discard them all. Style, colour, trail and effect spins unlock items for every character; a duplicate refunds 10 VP. Equip them in the Locker tab. Item odds are Common 58%, Rare 29%, Epic 10% and Legendary 3% (renormalized over the rarities a banner has). VP packs are sold as Developer Products (Shop tab, right column); set their ids in `Config.Shop.Packs`. Until then they show "Soon", and in Studio they grant their VP for free so the flow can be tested. Your active character is locked while your match is running (no allocation or keeping a roll), which keeps client prediction identical to the server.
...
src/shared/            deterministic code used by both server and clients
  ...
  Characters.lua       tiers, builds, rolled caps, heights, stat curves, jump heights
  Spins.lua            spin odds, unlockable items, colour and trail helpers
...
tools/                 checkers, the simulation suite, the SFX and icon generators
...
texlua tools/sim_test.lua       # 65 gameplay scenarios against the real shared code
```

The simulation suite checks the headline numbers (spike speeds, Thunder and Azure ranges, hitting points by tier, depth control, stamina and guard breaks, touch rules, sets, serves, blocks, rolled caps and allocation, spin odds, deuce, rotation edits, formation fill, bot skill by tier) and that client prediction is bit-identical to the server.
